 ...
 * Date : 2015-11-01
 */
public class GroupRelationFragment extends BaseFragment{
            ...
            ApiManager.getApi()
                    ...
                    .subscribe(new Subscriber<List<RelationEntity>>() {
                        ...
                        public void onNext(List<RelationEntity> relationEntities) {
                            RelationEntity re;
                            RelationEntity rey = new RelationEntity();
                            for (RelationEntity rety:relationEntities){
                                if (rety.getId().equals(mGroupId)){
                                    rey.setId(rety.getId());
                                    rey.setAttention(rety.getAttention());
                                    rey.setImageurl(rety.getImageurl());
                                    rey.setIsgroup(rety.getIsgroup());
                                    rey.setName(rety.getName());
                                    rey.setParentid(rety.getParentid());
                                    break;
                                }
                            }
                            for (int i=0; i<relationEntities.size(); i++)
                            {
                                ...
                                if (re.getIsgroup().equals("1"))
                                    ...
                                    isgroup = false;
                                //  自己的子类
                                if (re.getParentid().equals(rey.getId()) ||
                                        //自己的父类
                                        re.getId().equals(rey.getParentid()) ||
                                        //父类的子类 并且不为群
                                        re.getParentid().equals(rey.getParentid())&&re.getIsgroup().equals("0") ||
                                        //自己
                                        re.getId().equals(rey.getId())
                                        ){
