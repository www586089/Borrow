...
public abstract class RecycleBaseAdapter extends RecyclerView.Adapter<RecycleBaseAdapter.ViewHolder> {
    ...
    private void onBindFooterViewHolder(ViewHolder holder, int position) {
        ...
        if (!loadMoreHasBg()) {
        //gaobo update
//            vh.loadmore.setBackground(null);
        }
        switch (getState()) {
            case STATE_LOAD_MORE:
            ...
            case STATE_NETWORK_ERROR:
                ...
                if (TDevice.hasInternet()) {
                    ...
                    vh.text.setText(Application.string(R.string.tip_load_data_error));
                } else {
                    vh.text.setText(Application.string(R.string.tip_network_error));
                }
                break;
            default:
                vh.loadmore.setVisibility(View.GONE);
